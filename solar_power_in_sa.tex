...
\begin{itemize}
\item \textbf{\emph{Global Horizontal Irradiance} (\acsu{GHI})} in \si{\kilo\watt\hour\per\square\metre\per\year} or \si{\watt\per\square\metre}: \ac{GHI} is the total amount of shortwave radiation received from above by a horizontal surface. It includes direct (beam) and a diffuse (scattered) irradiation. This value is of particular interest when designing PV or solar water heater systems with a fixed inclined angle.
\item \textbf{\emph{Direct Normal Irradiance} (\acsu{DNI})} in \si{\kilo\watt\hour\per\square\metre\per\year} or \si{\watt\per\square\metre}: \ac{DNI} is the amount of solar radiation received per unit area by a surface that is always held perpendicular (or normal) to the rays that come in a straight line from the direction of the sun at its current position in the sky. Diffuse irradiation is totally excluded from the DNI. This quantity is relevant for installations that track the position of the sun.
\item \textbf{\emph{Diffuse Horizontal Irradiance} (\acsu{DHI})} in \si{\kilo\watt\hour\per\square\metre\per\year} or \si{\watt\per\square\metre}: \ac{DHI} is the amount of radiation received per unit area by a surface that does not arrive on a direct path from the sun, but has been scattered by molecules and particles in the atmosphere and comes equally from all directions.
\end{itemize}
